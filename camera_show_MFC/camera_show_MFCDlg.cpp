...
void Ccamera_show_MFCDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_X_COOR, n_X_COOR);
	DDX_Text(pDX, IDC_Y_COOR, n_Y_COOR);
	DDX_Text(pDX, IDC_3D_COOR, st_3D_COOR);
}
...
BEGIN_MESSAGE_MAP(Ccamera_show_MFCDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_3D_MEASURE, &Ccamera_show_MFCDlg::OnBnClicked3dMeasure)
END_MESSAGE_MAP()
		...
		case 5:
			{
				...
				//showImg(disp,IDC_DISPARITY);//修改的代码
				break;
			}
	}
...
void Ccamera_show_MFCDlg::OnBnClickedShowdisparity()
{
	...
	*/
	...
	disp.convertTo(disp8, CV_8U, 255 / ((numberOfDisparities * 16 + 16)*16.));//计算出的视差是CV_16S格式
	showImg(disp8,IDC_DISPARITY);
	//showImg(disp,IDC_DISPARITY);//修改的
	SetTimer(5,30,NULL);
}
...
void Ccamera_show_MFCDlg::OnBnClickedReadfile()
{
	...
	if(strPath=="") return;
	strPath.Replace("\\","\\\\"); //判断一下是否打开了文件，否则就return，不添加这个判断如果没有打开文件会报错
...
void Ccamera_show_MFCDlg::OnBnClicked3dMeasure()
{
	 UpdateData(TRUE);	//将控件中的值传到对应的变量中
	 if(n_X_COOR==0||n_Y_COOR==0)
	 {
		 AfxMessageBox("请先输入需要测量的像素坐标");
		 return;
	 }
	 int x=n_X_COOR;
	 int y=n_Y_COOR;
